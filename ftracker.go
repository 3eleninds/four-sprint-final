...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		distance := distance(action)                                               // вызовите здесь необходимую функцию
		speed := smingMeanSpeed(lengthPool, countPool, duration)                   // вызовите здесь необходимую функцию
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedWalk := meanSpeed(action, duration) * kmhInMsec // перевод скорости в м/с
	height = height / cmInM                              // перевод роста в метры

	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(speedWalk, 2)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH
...
// duration float64 — длительность тренировки в часах.
func smingMeanSpeed(lengthPool, countPool int, duration float64) float64 {
	if duration == 0 {
		return 0
	}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	return (smingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
